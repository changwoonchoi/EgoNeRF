...
if __name__ == "__main__":
	...
	with open(os.path.join(TEXT_FOLDER,"cameras.txt"), "r") as f:
		...
		for line in f:
			...
			# handle Spherical Camera model
			is_spherical = els[1] == "OPENCV_SPHERICAL"

			fl_x = float(els[4]) if not is_spherical else 1
			fl_y = float(els[4]) if not is_spherical else 1
			k1 = 0
			k2 = 0
			p1 = 0
			p2 = 0
	...
	if is_spherical:
		# calculate center of poses
		totp = np.array([0.0, 0.0, 0.0])
		for f in out["frames"]:
			totp += f["transform_matrix"][0:3, 3]
		totp = totp / len(out["frames"])
	else:
		totw = 0.0
		totp = np.array([0.0, 0.0, 0.0])
		for f in out["frames"]:
