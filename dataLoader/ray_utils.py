...
def get_rays(directions, c2w, roi=None):
    """
    ...
    # crop directions if roi is given
    if roi is not None:
        h_begin, h_end, w_begin, w_end = roi
        H, W, _ = directions.shape
        directions = directions[int(h_begin*H):int(h_end*H), int(w_begin*W):int(w_end*W), :]
...
def sample_pdf(bins, weights, N_samples, is_train=True):
