...
class TensorVM(TensorBase):
    def __init__(self, aabb, gridSize, device, coordinates, **kargs):
        super(TensorVM, self).__init__(aabb, gridSize, device, coordinates, **kargs)
...
class TensorVMSplit(TensorBase):
    def __init__(self, aabb, gridSize, device, coordinates, **kargs):
        # super(TensorVMSplit, self).__init__(aabb, gridSize, device, coordinates, **kargs)
        super().__init__(aabb, gridSize, device, coordinates, **kargs)
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        for idx_plane in range(len(self.density_plane)):
            ...
            line_coef_point = F.grid_sample(self.density_line[idx_plane], coordinate_line[[idx_plane]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1])
            sigma_feature = sigma_feature + F.relu(torch.sum(plane_coef_point * line_coef_point, dim=0))
        return sigma_feature
    ...
    def sample_envmap(self, dir_sampled):
    ...
    def shrink(self, new_aabb):
        print("====> shrinking ...")
        t_l, b_r = self.coordinates.get_normalized_range(new_aabb)
        t_l, b_r = t_l * self.gridSize, b_r * self.gridSize
        ...
        newSize = b_r - t_l
        self.aabb = new_aabb
        self.coordinates.update_aabb(new_aabb)
        self.update_stepSize((newSize[0], newSize[1], newSize[2]))


class TensorCP(TensorBase):
    def __init__(self, aabb, gridSize, device, coordinates, **kargs):
        super(TensorCP, self).__init__(aabb, gridSize, device, coordinates, **kargs)
